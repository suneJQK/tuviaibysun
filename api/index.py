...
WEB_FEATURES = ROOT / "web" / "features.js"
...
app.add_middleware(CORSMiddleware, allow_origins=ALLOWED_ORIGINS, allow_credentials=False, allow_methods=["GET", "POST"], allow_headers=["Content-Type", "X-Diagnostic-Token"])

class BirthRequest(BaseModel):
    ngay: int = Field(ge=1, le=31); thang: int = Field(ge=1, le=12); nam: int = Field(ge=1800, le=2200)
    gio_sinh: str | int; gioi_tinh: str; ten: str = ""; duong_lich: bool = True; time_zone: float = 7.0
    nam_xem: int | None = Field(default=None, ge=1800, le=2200); thang_xem: int | None = Field(default=None, ge=1, le=12)
    ngay_xem: int | None = Field(default=None, ge=1, le=31); gio_xem: int | None = Field(default=None, ge=1, le=12)

class AskRequest(BirthRequest):
    question: str = Field(min_length=1, max_length=8000); year: int | None = Field(default=None, ge=1800, le=2200); provider: str | None = None
...
def _available_ai_modes() -> list[dict[str,str]]:
    ...
    for path in sorted(AI_MODE_DIR.glob("*.txt")):
        text=path.read_text(encoding="utf-8").strip(); first_line=next((x.strip() for x in text.splitlines() if x.strip()), "")
...
def _load_ai_mode(mode_id: str|None)->tuple[str,str]:
    ...
    wanted=(mode_id or "standard").strip().lower(); path=AI_MODE_DIR/f"{wanted}.txt"
    if not path.exists(): path=AI_MODE_DIR/f"{modes[0]['id']}.txt"; wanted=modes[0]["id"]
...
def _slim_ai_context(context:dict[str,Any])->dict[str,Any]:
    payload=context.get("ai_payload"); slim=dict(context)
    if isinstance(payload,dict) and payload.get("selected_palaces") is not None: slim.pop("palaces",None)
    if slim.get("matched_cach_cuc")==((slim.get("confirmed_cach_cuc") or {}).get("items")): slim.pop("matched_cach_cuc",None)
    return slim
...
def _prepare_chart(req:BirthRequest)->dict[str,Any]:
    ...
    analyzed=analyze_chart(chart); analyzed.setdefault("input",{})["lich"]="Dương lịch" if req.duong_lich else "Âm lịch"
    return normalize_engine_chart(analyzed)

def _view_year(req:BirthRequest,explicit_year:int|None=None)->int:
    if explicit_year is not None: return int(explicit_year)
    if req.nam_xem is not None: return int(req.nam_xem)
    return datetime.now(timezone(timedelta(hours=float(req.time_zone)))).year

def _view_args(req:BirthRequest,default_year:int|None=None)->dict[str,Any]: return {"year":_view_year(req,default_year),"month":req.thang_xem,"day":req.ngay_xem,"hour":req.gio_xem}

def _profile_id(req:BirthRequest)->str:
    name=req.ten.strip() or "—"; birth_date=f"{req.ngay:02d}/{req.thang:02d}/{req.nam:04d}"
    key="|".join([name.casefold(),birth_date,str(req.gio_sinh).strip().casefold(),req.gioi_tinh.strip().casefold(),"Dương lịch" if req.duong_lich else "Âm lịch",str(req.time_zone)])
...
def _save_profile(req:BirthRequest)->dict[str,Any]:
    try:
        ...
        created_at=datetime.now(timezone.utc).astimezone(VN_TZ).isoformat(timespec="seconds"); name=req.ten.strip() or "—"; birth_date=f"{req.ngay:02d}/{req.thang:02d}/{req.nam:04d}"
        return save_user_profile(user_id=_profile_id(req),name=name,ngay_sinh=birth_date,gio_sinh=str(req.gio_sinh),gioi_tinh=req.gioi_tinh,lich="Dương lịch" if req.duong_lich else "Âm lịch",time_zone=req.time_zone,created_at=created_at)
    except Exception as exc: return {"saved":False,"error":f"{type(exc).__name__}: {exc}"}

def _assert_ai_payload_sync(calc:dict[str,Any],ai_context:dict[str,Any])->None:
    van=calc.get("van") or {}; authoritative=van.get("tieu_van") or {}; synced=(van.get("sync_contract") or {}).get("tieu_van_cung_so")
    if synced!=authoritative.get("cung_so"): raise ValueError("Dữ liệu Tiểu vận nội bộ không đồng bộ")
    if (ai_context.get("van_han") or {}).get("tieu_van")!=authoritative: raise ValueError("AI context và Tiểu vận authoritative không đồng bộ")
    palace_items=(ai_context.get("palaces") or {}).values() if isinstance(ai_context.get("palaces"),dict) else (ai_context.get("palaces") or [])
...
def _ai_context_for_request(chart:dict[str,Any],calc:dict[str,Any])->dict[str,Any]:
    ...
    _assert_ai_payload_sync(calc,context); return context

@app.get("/",response_class=HTMLResponse)
def root()->HTMLResponse:
    if not WEB_INDEX.exists(): raise HTTPException(status_code=500,detail="Thiếu web/index.html")
    html=WEB_INDEX.read_text(encoding="utf-8")
    if WEB_FEATURES.exists():
        tag='<script src="/web/features.js"></script>'
        if tag not in html: html=html.replace("</body>",tag+"</body>")
    return HTMLResponse(html)

@app.get("/web/features.js")
def web_features()->FileResponse:
    if not WEB_FEATURES.exists(): raise HTTPException(status_code=404,detail="Thiếu web/features.js")
    return FileResponse(WEB_FEATURES,media_type="application/javascript")
...
def google_sheets_test(request:Request)->dict[str,Any]:
    ...
    if not diagnostic_token or request.headers.get("X-Diagnostic-Token")!=diagnostic_token: raise HTTPException(status_code=403,detail="Forbidden")
    try:
        ...
        logger.exception("Google Sheets diagnostic failed"); return {"ok":False,"error":"Diagnostic failed"}
...
def lap_so(req:BirthRequest)->dict[str,Any]:
    try:
        ...
        logger.exception("Chart calculation failed"); raise HTTPException(status_code=400,detail="Không thể lập lá số với dữ liệu đã gửi.") from exc
...
def luan_giai(req:AskRequest,request:Request)->dict[str,Any]:
    try:
        ...
        chart.pop("ai_context",None); return {"chart":chart,"van":calc.get("van",{}),"answer":answer,"ai_status":"ok","ai_mode":mode_id,"ai_provider":selected_provider,"year":viewing_year}
    except Exception as exc:
        logger.exception("AI interpretation failed"); raise HTTPException(status_code=500,detail="Không thể hoàn tất luận giải.") from exc
